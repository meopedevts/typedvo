...
class Delegate<T>: ReadWriteProperty<DynamicVO, T?> {
    override fun getValue(thisRef: DynamicVO, property: KProperty<*>): T? {
        ...
        return when (property.returnType) {
            Short::class.createType(nullable = true) -> thisRef.asBigDecimal(nameProperty)?.toInt()?.toShort()
            Int::class.createType(nullable = true) -> thisRef.asBigDecimal(nameProperty)?.toInt()
            BigDecimal::class.createType(nullable = true) -> thisRef.asBigDecimal(nameProperty)
            String::class.createType(nullable = true) -> thisRef.asString(nameProperty)
            else -> thisRef.getProperty(nameProperty)
        } as T?
    }
